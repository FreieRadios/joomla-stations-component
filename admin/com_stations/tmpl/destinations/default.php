...
{
	$saveOrderingUrl = 'index.php?option=com_stations&task=destinations.saveOrderAjax&tmpl=component&' . Session::getFormToken() . '=1';
	HTMLHelper::_('draggablelist.draggable');
}
?>

<form action="<?php echo Route::_('index.php?option=com_stations&view=destinations', false); ?>" method="post" name="adminForm" id="adminForm">
	<div class="row">
		<div class="col-md-12">
					...
					<table class="table" id="fieldList">
						...
						<tbody <?php if ($saveOrder) : ?> class="js-draggable" data-url="<?php echo $saveOrderingUrl; ?>" data-direction="<?php echo strtolower($listDirn); ?>" data-nested="true"<?php endif; ?>>
							<?php foreach ($this->items as $i => $item) : ?>
								...
								<?php $canEdit    = $user->authorise('core.edit', $component . '.destination.' . $item->id); ?>
								<?php $canCheckin = $user->authorise('core.admin', 'com_checkin') || $item->checked_out === $userId || is_null($item->checked_out); ?>
								<?php $canEditOwn = $user->authorise('core.edit.own', $component . '.destination.' . $item->id) && $item->created_by === $userId; ?>
								...
								<tr class="row<?php echo $i % 2; ?>" data-draggable-group="1" item-id="<?php echo $item->id; ?>">
									...
									<td class="text-center">
										<?php echo HTMLHelper::_('jgrid.published', $item->published, $i, 'destinations.', $canChange, 'cb'); ?>
									</td>
									<th scope="row">
									...
									<td class="small d-none d-md-table-cell">
										<?php echo HTMLHelper::_('date', $item->created, 'Y-m-d'); ?>
									</td>
									<td class="small d-none d-md-table-cell">
