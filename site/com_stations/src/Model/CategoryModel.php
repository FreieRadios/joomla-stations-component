 ...
 * @license     GNU General Public License version 2 or later; see LICENSE.txt General Public License version 2 or later; see LICENSE.txt
 */
...
/**
 * Single item model for a station
 *
 * @package     Joomla.Site
 * @subpackage  com_stations
...
class CategoryModel extends ListModel
{
    ...
    /**
     * Array of stations in the category
     *
     * @var    \stdClass[]
     */
    ...
    /**
     * The list of other station categories.
     *
     * @var    array
     */
 ...
 * @copyright   Copyright (C) 2005 - 2022 Open Source Matters, Inc. All rights reserved.
    ...
    /**
     * Method to get a list of items.getInstance
     *
    ...
    public function getItems()
    {
        ...
        $model = $this->bootComponent('com_stations')->getMVCFactory()
            ->createModel('Destinations', 'Site', ['ignore_request' => true]);
        $categoryId = $this->getState('category.id');
        $model->setState('filter.category_id', $this->getState('category.id'));
        ...
        for ($i = 0, $n = count($items); $i < $n; $i++) {
            ...
            if ($this->getState('load_tags', true)) {
                ...
                $this->tags->getItemTags('com_stations.station', $item->id);
            }
        }
        return $items;
    }
    ...
    protected function getListQuery()
    {
        ...
        $query->select($this->getState('list.select', 'a.*'))
            ...
            ->from($db->quoteName('#__stations_destinations', 'a'))
    ...
    protected function populateState($ordering = null, $direction = null)
    {
        ...
        $params = ComponentHelper::getParams('com_stations');
        ...
        $numberOfStationsToDisplay = $mergedParams->get('stations_display_num');

        if ($format === 'feed') {
            $limit = $app->get('feed_limit');
        } else if (isset($numberOfStationsToDisplay)) {
            $limit = $numberOfStationsToDisplay;
        } else {
        ...
        $search = $app->getUserStateFromRequest('com_stations.category.list.' . $itemid . '.filter-search', 'filter-search', '', 'string');
        ...
        if ((!$user->authorise('core.edit.state', 'com_stations')) && (!$user->authorise('core.edit', 'com_stations'))) {
    ...
    public function getCategory()
    {
        if (!is_object($this->_item)) {
            ...
            $categories = Categories::getInstance('Stations');
    ...
    {
        if (!is_object($this->_item)) {
            $this->getCategory();var_dump($query->__toString());
        }
